SELECT path, key FROM (
    SELECT row_number() over (partition by file.path ORDER BY patch desc) rows, file.patch, file.path, data.checksum, data.uncompressed_size, data.key FROM files file
